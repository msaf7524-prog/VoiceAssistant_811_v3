...
class GroqClient:
    ...
    def ask(self, prompt):
        if not self.is_ready:
            raise ValueError("مفتاح API غير موجود، يرجى أدخال المفتاح")
        ...
        if response.status_code == 401:
            raise ValueError("مفتاح API غير صالح، يرجى إنشاؤه مجدداً من Groq")
        elif response.status_code != 200:
            raise ValueError(f"خطأ في الاتصال: {response.status_code}")
...
class VoiceAssistant811(BoxLayout):
    ...
    def on_button_click(self, instance):
        ...
        if ":" in cleaned_key:
            cleaned_key = cleaned_key.split(":")[-1].strip()

        if not cleaned_key:
            self.update_status("خطأ: الصق مفتاح API", color=(1, 0.2, 0.2, 1), state="idle")
            return
