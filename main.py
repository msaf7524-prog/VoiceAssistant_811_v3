...
GEMINI_API_KEY = os.environ.get("AQ.Ab8RN6KkUgKsAetuELPjj2IvhP6zWXTXtu8tkv3sCWDeoSBpLg", "AQ.Ab8RN6KkUgKsAetuELPjj2IvhP6zWXTXtu8tkv3sCWDeoSBpLg")
...
class MainScreen(BoxLayout):
    ...
    def update_timer(self, dt):
        if self.remaining_seconds > 0:
            ...
            self.timer_button_text = ar(f"نشط: {hrs:02d}:{mins:02d}:{secs:02d}")
        else:
            self.toggle_timer()

    def start_voice_interaction(self):
        self.circle_color = [0, 0.9, 0.46, 1]
        self.status_text = ar("جاري الإنصات والمعالجة...")
        threading.Thread(target=self.call_gemini_api, args=("مرحباً المساعد 811",), daemon=True).start()
    ...
    def call_gemini_api(self, prompt_text):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                Clock.schedule_once(lambda dt: self.update_ui_response(reply))
            else:
                err_msg = f"خطأ: {response.status_code}"
                Clock.schedule_once(lambda dt: self.update_ui_error(err_msg))
        except Exception as e:
            Clock.schedule_once(lambda dt: self.update_ui_error("حدث خطأ في الاتصال"))

    def update_ui_response(self, reply):
        self.circle_color = [0, 0.82, 1, 1]
    ...
    def open_settings(self):
        ...
        content.add_widget(Label(text=ar(f"حساسية الصوت: {int(self.mic_sensitivity * 100)}%")))
        
        slider = Slider(min=0, max=1, value=self.mic_sensitivity)
        slider.bind(value=self.on_slider_change)
        content.add_widget(slider)

        bt_box = BoxLayout(spacing=10)
        bt_box.add_widget(Label(text=ar("تفعيل البلوتوث:")))
        bt_switch = Switch(active=self.bluetooth_enabled)
        bt_switch.bind(active=self.on_switch_toggle)
        bt_box.add_widget(bt_switch)
        content.add_widget(bt_box)

        close_btn = Button(text=ar("إغلاق الإعدادات"), size_hint_y=0.4)
        content.add_widget(close_btn)

        popup = Popup(title=ar("إعدادات المساعد 811"), content=content, size_hint=(0.85, 0.55))
